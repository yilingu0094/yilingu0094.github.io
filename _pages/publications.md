...
**Background:** The nonconvex-nonconcave minmax problem is an important issue arising in operations management and machine learning theory. For example, many operational objectives are formulated in a minmax setting $\min_{x}\max_{y}f(x,y)$, where $x$ and $y$ are real-valued vectors and $f$ is neither (necessarily) convex in $x$ nor (necessarily) concave in $y$. This problem can be intractable without strong assumptions on $f$. And given the dataset provided always in a large scale and many objectives can be expressed in the form $\min_{x\in {\mathbb{R}}^{n}}f(x) = \Sigma_{i=1}^{N}f_{i}(x)$

However, it might be even trickier with complicated constraints for $(x,y)$. 
