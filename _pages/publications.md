...
where $\Gamma_p = \sum_{i}^{m_k}\frac{\left\Vert x_i^k-A^{k,p}\right \Vert -r_p}{\|\left\Vert x_i^k-A^{k,p}\right \Vert -r_p\|}$ for $p^{\textrm{th}}$ iteration (specifically we rule that $\Gamma_0 = m_k$). When the iteration of $A^{k}$ reaches the point where $(7)$ is satisfied, it suggests that we should <span style="color:DarkGoldenRod">stop</span> our iteration because we won't get many <span style="color:DarkGoldenRod">marginal</span> covered points even if we keep raising the radius of the prototype's decision region much. As a result, it stops us from increasing the radius too much and making the model <span style="color:DarkGoldenRod">non-robust</span>. While $(8)$ is fulfilled, the sample points covered by the prototype shift from a sparse region to a condense one, which allows to discriminate between the <span style="color:DarkGoldenRod">sparsity</span> of samples with prototypes.
